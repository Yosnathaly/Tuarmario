...
describe("Operaciones CRUD de Tuarmario", () => {
    ...
    it("Test delete", async () => {
        ...
        const response = await request(server).delete(`/productos/${nonExistentId}`).set('Authorization', 'Bearer someToken').send();
        const status = response.statusCode;
    ...
    it("Test post", async () => {
        ...
        const response = await request(server).post("/productos").send(newProducto);
        const status = response.statusCode;
        const body = response.body;
    ...
    it("Test put", async () => {
        ...
        const response = await request(server).put(`/productos/${productoId}`).send(payload);
        const status = response.statusCode;
